...
const Calculator = {
    ...
    //This will hold he operator, we set it to null for now.
    operator: null,
};
...
function Input_Digit(digit) {
    ...
    //This checks to see if the Wait_Second_Operand is true and sets Display_Value to the key that was clicked on.
...
function Input_Decimal(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        //We are saying that if the Display_Value does not contain a decimal point then we want to add a decimal point.
        Calculator.Display_Value += dot;
    }
}

//This sections handles operators
function Handle_Operator(Next_Operator) {
    const {First_Operand, Display_Value, operator} = Calculator;
    //When an operator key is pressed we convert the current number display on the screen to a number
